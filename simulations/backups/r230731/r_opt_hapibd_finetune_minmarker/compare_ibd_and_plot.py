...
r = 0.01 / ([15000] * 2 + [1000000] * 2)[which]
len_bp = ([15000 * 100] * 2 + [1000000 * 60] * 2)[which]
len_cm = ([100.0] * 2 + [60.0] * 2)[which]
...
genome_set_id = [10000, 20000, 60001, 60002][which]
# copy true ibd
for chrno in range(1, 15):
    # -------------CHANGE HERE AS NECESSARY
...
for p in Path(f"res/{genome_set_name}/ibd/hapibd").glob("*"):
...
for col, cat in enumerate(cats):
    ...
    ax.set_xticklabels(markers, fontsize=7)
    ax.set_yticks(np.arange(maxgaps.size))
    if col == 0:
        ax.set_yticklabels(maxgaps)
    else:
...
demog = ["sp", "mp", "uknogc", "ukgc"][which]
id_desc = f"minmarker_maxgap_{demog}"
