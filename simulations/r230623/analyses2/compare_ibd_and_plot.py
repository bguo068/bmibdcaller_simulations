...
# copy true ibd
for chrno in range(1, 15):
    src_fn = f"../res_mm8/sp_neu/ibd/tskibd/10000_{chrno}_tskibd.ibd"
    dst_fn = f"tskibd/{chrno}.ibd"
    Path(dst_fn).parent.mkdir(parents=True, exist_ok=True)
    copyfile(src_fn, dst_fn)

# copy inferred ibd
for p in Path("../res_mm8/sp_neu/ibd/hapibd").glob("*"):
...
df["LenBinStart"] = pd.Categorical(
    df.LenBinStart, categories=["3", "4", "6", "10", "18", "genome_wide"]
)

fn = 1 - df.pivot(columns="LenBinStart", index="MinMarkers", values=["RateAOverlapByB"])
fp = 1 - df.pivot(columns="LenBinStart", index="MinMarkers", values=["RateBOverlapByA"])
fn.columns = [i[1] for i in fn.columns]
fp.columns = [i[1] for i in fp.columns]
