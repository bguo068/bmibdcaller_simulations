...
parser.add_argument("--raw_ibd_files", nargs="+", type=str, required=True)
parser.add_argument("--outdir", type=str, required=True)
args = parser.parse_args()

raw_ibd_files = args.raw_ibd_files
outdir = args.outdir

# read ibd file
genome = Genome.get_genome("simu_14chr_100cm")
ibd = IBD(genome, label="")
ibd.read_ibd(ibd_fn_lst=raw_ibd_files)

# make total ibd matrix
mat = ibd.make_ibd_matrix()

# find samples not highly related
unrelated_samples = ibd.get_unrelated_samples(mat)

# print
print(
    f"""
      No. sample before: {ibd.get_samples_shared_ibd().size}; 
      No. Sample after: {unrelated_samples.size}
      Max gw IBD: {np.max(mat.values)}
      """
)

# remove samples from ibd dataframe
ibd.subset_ibd_by_samples(unrelated_samples)

# write to file
for chr, df in ibd._df.groupby("Chromosome"):
    ibd_fn = f"{outdir}/{chr}.ibd"
    columns = ["Id1", "Id2", "Start", "End", "HasMutation", "Ancestor", "Tmrca"]
    df[columns].to_csv(ibd_fn, sep="\t", index=None)
